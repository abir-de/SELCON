...
class DeepSelection():
    ...
    def train_model(self, x_trn, y_trn, x_val, y_val, fraction, delt = [0.3], x_tst = None, y_tst = None,\
                    num_epochs=5000, default = False, ebud=None):
        ...
        main_model = TwoLayerNet(M,self.hidden_units)
        ...
        gamma = torch.ones(len(idxs),device=self.device)
        #while(True):
        for i in range(num_epochs):#curr_epoch):#num_epochs):
            ...
            if ((i + 1) % self.select_every == 0):
                ...
                step = 0
                w_exp_avg = torch.zeros(self.hidden_units+1,device=self.device)
                w_exp_avg_sq = torch.zeros(self.hidden_units+1,device=self.device)

                #print(exp_avg,exp_avg_sq)

                d_sub_idxs = fsubset_d.return_subset(clone_dict,sub_epoch,current_idxs,\
                    bud,self.batch_size,step,w_exp_avg,w_exp_avg_sq)
                ...
                print("First 10 subset indices: ", idxs[:10])
                #print(idxs[:10])
                np.random.seed(42)
                np_sub_idxs = np.array(idxs)
                np.random.shuffle(np_sub_idxs)
    ...
    def train_model_fair(self, x_trn, y_trn, x_val, y_val, fraction,  delt = [0.3], x_tst = None, y_tst = None,\
                    num_epochs=2000, default = False, bud=None, is_time = False):
        ...
        for i in range(num_epochs):
            ...
            if ((i + 1) % self.select_every == 0):
                ...
                d_sub_idxs = fsubset_d.return_subset(clone_dict,sub_epoch,current_idxs,alpha_orig,bud,\
                    self.batch_size,step,w_exp_avg,w_exp_avg_sq,a_exp_avg,a_exp_avg_sq)#,main_optimizer,dual_optimizer)
