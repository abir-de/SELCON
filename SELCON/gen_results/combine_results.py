...
if typeOf == "budget":
    ...
    fractions = [ int(1000*round(float(i), 3)) for i in in_dir]
    fractions, in_dir = zip(*sorted(zip(fractions, in_dir)))

    logfile = open(os.path.join(directory, 'combined_'+data_name+'_'+frac+'_all_bud.txt'), 'w')

    selection =[]
    for di in in_dir:
        temp = [int(i) for i in os.listdir(os.path.join(directory, di,frac))]
    ...
    for sel in selection:
        ...
        for delta in range(len(in_dir)):
            ...
            file_path = os.path.join(directory,in_dir[delta],frac,str(sel),data_name+'.txt')
            ...
            with open(file_path) as fp:
                ...
                while line:
                    ...
                    if tim[0] in ["Subset","Random","Full","Facility","Glister","CRAIG"]:
